...
//ubah desember
    ...
    {
        $hasil = $this->db->query("SELECT rate,tanggal from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' and tanggal = '$inv_date'");
        return $hasil->row();
    }
    ...
    function get_kode_book_invoice($kode_inv)
    {
        ...
        $q = $this->db->query("SELECT no_invoice, MAX(LEFT(no_invoice, 4)) AS kd_max FROM tbl_book_invoice WHERE YEAR(tgl_book_inv) = YEAR(CURRENT_DATE()) AND 
          MONTH(tgl_book_inv) = MONTH(CURRENT_DATE()) ");
        //
        $kd = "";
        if ($q->num_rows() > 0) {
    ...
    function cari_bi($dt_dari, $dt_sampai)
    {
        ...
        $hasil = $this->db->query("SELECT *from (SELECT a.doc_num,a.date,c.Id_Supplier,a.customer,b.id as id_bank,a.bank,a.akun,a.curr,
            if(a.curr = 'USD',if(((a.amount) - (select DISTINCT sum(k.amount) from tbl_alokasi k left join tbl_bankin_arcollection d on d.doc_num = k.doc_number where k.doc_number = a.doc_num)) is null,a.amount,Round((a.amount) - (select DISTINCT sum(k.amount) from tbl_alokasi k left join tbl_bankin_arcollection d on d.doc_num = k.doc_number where k.doc_number = a.doc_num),2)),if(((a.amount) - (select DISTINCT sum(k.eqp_idr - k.sisa) from tbl_alokasi k left join tbl_bankin_arcollection d on d.doc_num = k.doc_number where k.doc_number = a.doc_num)) is null,a.amount,((a.amount) - (select DISTINCT sum(k.eqp_idr - k.sisa) from tbl_alokasi k left join tbl_bankin_arcollection d on d.doc_num = k.doc_number where k.doc_number = a.doc_num)))) as amount,
                a.rate,
...
function approve_invoice($id, $created_by, $created_date)
{
    $hasil = $this->db->query("UPDATE tbl_book_invoice SET status = 'FIRST APPROVED', first_approve_by = '$created_by', first_approve_date = '$created_date' WHERE id = '$id' ");
...
function approve_invoice_second($id, $created_by, $created_date)
{
    ...
    $query = $this->db->query("SELECT profit_center, tgl_inv FROM tbl_book_invoice WHERE id = '$id'");
    $row = $query->row();
...
function load_user_access()
{
    $hasil = $this->db->query("SELECT b.id,b.user, a.menu, b.base_url
       FROM tbl_user_role AS a INNER JOIN tbl_user_access AS b ON a.id = b.menu_id where a.status = 'Y'");
    return $hasil->result_array();
}
...
function load_user_access_1($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       ...
       WHERE b.user = '$user' AND a.menu_status = 'main_menu_1' and a.status = 'Y'");
    return $hasil->result_array();
}

function load_user_access_2($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status, a.ctg_menu
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'main_menu_2' and a.status = 'Y'");
    return $hasil->result_array();
}
...
function load_user_access_3($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'report' and a.status = 'Y'");
    return $hasil->result_array();
}
...
function load_user_access_4($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'main_menu_3' and a.status = 'Y'");
    return $hasil->result_array();
}
function load_user_access_5($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'kwitansi' and a.status = 'Y'");
    return $hasil->result_array();
}
...
function load_user_access_6($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'alokasi' and a.status = 'Y'");
    return $hasil->result_array();
}
...
function load_user_access_7($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'master' and a.status = 'Y'");
    return $hasil->result_array();
}
...
function load_user_access_reverse($user)
{
    $hasil = $this->db->query("SELECT b.user, a.menu, b.base_url, a.menu_status
       FROM tbl_user_role AS a INNER JOIN 
       tbl_user_access AS b ON a.id = b.menu_id
       WHERE b.user = '$user' AND a.menu_status = 'doc_reverse' and a.status = 'Y'");
    return $hasil->result_array();
}
...
function get_rate()
{
    ...
    $query = $this->db->query("SELECT rate FROM masterrate WHERE id IN (SELECT MAX(id) FROM masterrate where v_codecurr = 'HARIAN' AND curr = 'USD')");
    foreach ($query->result() as $data) {
        $hasil = ($data->rate);
        $hasil2 = '1';
        if ($query->num_rows() > 0) {
...
{
    // Gabungkan invoice IDR & USD sekaligus, independen dari Payment With ($pwith) --
    // sebelumnya invoice yang currency-nya beda dari $pwith tersaring habis (tidak
    // pernah muncul di daftar), jadi bayar lintas-currency (IDR bayar USD / USD bayar
    // IDR) tidak mungkin. amount1 (yang akhirnya tersimpan ke tbl_alokasi_detail.amount)
    // tetap native currency invoice masing-masing supaya sisa outstanding invoice ke
    // depannya tetap konsisten dihitung; konversi ke currency pembayaran (bagi/kali rate)
    // dilakukan di JS (cari_data_invoic() & modal_get_amount()), bukan di sini.
    $hasil = $this->db->query("
(SELECT DISTINCT a.no_proforma_invoice AS no_invoice, DATE_FORMAT(a.tgl_proforma_inv, '%Y-%m-%d') AS inv_date, f.top, if(g.kontrabon_date is null, DATE_ADD(DATE_FORMAT(a.tgl_proforma_inv, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(DATE_FORMAT(g.kontrabon_date, '%Y-%m-%d'), INTERVAL f.top DAY)) AS duedate, UPPER(b.supplier) AS customer, e.curr,
          ...
          WHERE a.tgl_proforma_inv BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.id_customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(IF(a.value_dp = '0',a.total,a.value_dp), 0), round(((IF(a.value_dp = '0',a.total,a.value_dp)) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_proforma_invoice)), 0)) != '0' and e.curr = 'IDR' ORDER BY a.id) union
          ...
          tbl_alokasi as i on h.no_alk = i.no_alk 
          WHERE a.tgl_dn BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(a.eqv_curr, 0), round(((a.eqv_curr) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_dn)), 0)) != '0' and a.to_curr = 'IDR' ORDER BY a.id) 
          union
          ...
          WHERE a.profit_center = 'NAG' and o.no_invoice is null and e.sj_date BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.id_customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(d.grand_total, 0), round(((d.grand_total) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_invoice)), 0)) != '0' and e.curr = 'IDR' ORDER BY a.id )
          union
          (SELECT DISTINCT a.no_invoice AS no_invoice, DATE_FORMAT(e.sj_date, '%Y-%m-%d') AS inv_date, f.top, if(g.kontrabon_date is null, DATE_ADD(DATE_FORMAT(e.sj_date, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(DATE_FORMAT(g.kontrabon_date, '%Y-%m-%d'), INTERVAL f.top DAY)) AS duedate, UPPER(b.supplier) AS customer, e.curr,
          a.status, a.id, if(h.amount is null,format(d.grand_total, 0), format(((d.grand_total) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_invoice)), 0)) AS amount, if(h.amount is null,round((d.grand_total), 0),round(((d.grand_total) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_invoice)), 0)) as amountrate, if(h.amount is null,format((d.grand_total), 0),format(((d.grand_total) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_invoice)), 0)) as amountrate1, if(h.amount is null,round(d.grand_total, 0), round(((d.grand_total) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_invoice)), 0)) AS amount1
          FROM  tbl_book_invoice AS a INNER JOIN 
          ...
          tbl_invoice_detail_knitting as e on a.id=e.id_book_invoice left JOIN 
          tbl_master_top AS f ON a.id_top = f.id left JOIN 
          tbl_duedate AS g ON a.id = g.id_invoice left join
          tbl_alokasi_detail as h on a.no_invoice = h.no_ref left join
          tbl_alokasi as i on h.no_alk = i.no_alk left join 
          ...
          WHERE o.no_invoice is null and e.sj_date BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(d.grand_total, 0), round(((d.grand_total) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_inv)), 0)) != '0' and e.curr = 'IDR' ORDER BY a.id )
...
(SELECT DISTINCT a.no_proforma_invoice AS no_invoice, DATE_FORMAT(a.tgl_proforma_inv, '%Y-%m-%d') AS inv_date, f.top, if(g.kontrabon_date is null, DATE_ADD(DATE_FORMAT(a.tgl_proforma_inv, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(DATE_FORMAT(g.kontrabon_date, '%Y-%m-%d'), INTERVAL f.top DAY)) AS duedate, UPPER(b.supplier) AS customer, e.curr,
      ...
      tbl_alokasi_detail as h on a.no_proforma_invoice = h.no_ref left join
      tbl_alokasi as i on h.no_alk = i.no_alk 
      WHERE a.tgl_proforma_inv BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.id_customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(IF(a.dp = '0',a.total,a.dp), 0), round(((IF(a.dp = '0',a.total,a.dp)) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_proforma_invoice)), 0)) != '0' and e.curr = 'USD' ORDER BY a.id) union
      ...
      tbl_alokasi_detail as h on a.no_dn = h.no_ref left join
      tbl_alokasi as i on h.no_alk = i.no_alk 
      WHERE a.tgl_dn BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(a.eqv_curr, 2), round(((a.eqv_curr) - (select sum(j.amount) from tbl_alokasi_detail j where no_ref = a.no_dn)), 2)) != '0' and a.to_curr = 'USD' ORDER BY a.id) 
      union
      (SELECT DISTINCT a.no_invoice AS no_invoice, DATE_FORMAT(e.sj_date, '%Y-%m-%d') AS inv_date, f.top, if(g.kontrabon_date is null, DATE_ADD(DATE_FORMAT(e.sj_date, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(DATE_FORMAT(g.kontrabon_date, '%Y-%m-%d'), INTERVAL f.top DAY)) AS duedate, UPPER(b.supplier) AS customer, e.curr,
      ...
      tbl_alokasi as i on h.no_alk = i.no_alk left join 
      saldoawal_ar as o on o.no_invoice = a.no_invoice
      WHERE a.profit_center = 'NAG' AND o.no_invoice is null and e.sj_date BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.id_customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(d.grand_total, 2),round((d.grand_total- (select sum(j.amount) from tbl_alokasi_detail j left join tbl_alokasi as k on j.no_alk = k.no_alk where no_ref = a.no_invoice)), 2)) != '0' and e.curr = 'USD' ORDER BY a.id)
      union
      (SELECT DISTINCT a.no_invoice AS no_invoice, DATE_FORMAT(e.sj_date, '%Y-%m-%d') AS inv_date, f.top, if(g.kontrabon_date is null, DATE_ADD(DATE_FORMAT(e.sj_date, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(DATE_FORMAT(g.kontrabon_date, '%Y-%m-%d'), INTERVAL f.top DAY)) AS duedate, UPPER(b.supplier) AS customer, e.curr,
      a.status, a.id, if(h.amount is null,format(d.grand_total, 2),format((d.grand_total- (select sum(j.amount) from tbl_alokasi_detail j left join tbl_alokasi as k on j.no_alk = k.no_alk where no_ref = a.no_invoice)), 2)) AS amount, if(h.amount is null,round((d.grand_total  * '$rate'), 2),round(((d.grand_total  * '$rate' - (select sum(j.amount) * '$rate' from tbl_alokasi_detail j left join tbl_alokasi as k on j.no_alk = k.no_alk where no_ref = a.no_invoice))), 0)) as amountrate, if(h.amount is null,format((d.grand_total  * '$rate'), 2),format(((d.grand_total  * '$rate' - (select sum(j.amount) * '$rate' from tbl_alokasi_detail j left join tbl_alokasi as k on j.no_alk = k.no_alk where no_ref = a.no_invoice))), 0)) as amountrate1, if(h.amount is null,round(d.grand_total, 2),round((d.grand_total- (select sum(j.amount) from tbl_alokasi_detail j left join tbl_alokasi as k on j.no_alk = k.no_alk where no_ref = a.no_invoice)), 2)) AS amount1
      FROM  tbl_book_invoice AS a INNER JOIN 
      ...
      tbl_alokasi as i on h.no_alk = i.no_alk left join 
      saldoawal_ar as o on o.no_invoice = a.no_inv
      WHERE o.no_invoice is null and e.sj_date BETWEEN '$dt_dari_invkwt' AND '$dt_sampai_invkwt' AND a.customer = '$id_customer' and a.status like '%APPROVED%' and if(h.amount is null,round(d.grand_total, 2),round((d.grand_total- (select sum(j.amount) from tbl_alokasi_detail j left join tbl_alokasi as k on j.no_alk = k.no_alk where no_ref = a.no_inv)), 2)) != '0' and e.curr = 'USD' ORDER BY a.id) 
      union
...
function cari_alokasi($dt_dari_kwt, $dt_sampai_kwt, $customer)
{
    ...
    if ($customer == "all_customer") {
     ...
     $hasil = $this->db->query("SELECT a.id, a.no_alk,a.tgl_alk,a.customer,b.supplier,a.doc_number,if(c.nama_bank is null, '-', c.nama_bank) as bank, if(a.account is null, '-',a.account) account,a.curr,if(rate = '0', '-',format(a.rate,0)) as rate,if(a.curr = 'IDR',format(a.amount, 0), format(a.amount, 2)) as amount,format(a.eqp_idr, 0) as eqp_idr,a.status,DATE_FORMAT(d.tanggal_input, '%Y-%m-%d') as tanggal_input,d.nama from tbl_alokasi AS a INNER JOIN mastersupplier AS b ON a.customer = b.id_supplier left join masterbank c on c.id = a.bank left join tbl_log d on d.doc_number = a.no_alk where a.tgl_alk BETWEEN '$dt_dari_kwt' AND '$dt_sampai_kwt' GROUP BY a.id ");
...
function report_alokasi_detail($id)
{
    ...
    $hasil = $this->db->query("SELECT a.no_alk, a.coa,IFNULL(c.nama_coa,'-') as coa_name, a.cost_center, IFNULL(CONCAT(d.no_cc,' - ',d.cc_name),'-') as cost_name, a.no_ref, a.ref_date, a.due_date, b.curr, if(b.curr = 'IDR',format(a.total, 2), format(a.total, 2)) as total, format(a.eqp_idr,2) as eqp_idr,format(a.amount, 2) as amount,format(round(a.total,2) - round(a.amount, 2),2) as ost, a.keterangan,e.supplier,if(b.rate = '0','-',b.rate) as rate from tbl_alokasi_detail a left join 
     mastercoa_v2 c on a.coa=c.no_coa LEFT JOIN 
     tbl_alokasi b on a.no_alk = b.no_alk left join
     mastersupplier AS e ON b.customer = e.id_supplier left join
     b_master_cc d on a.cost_center = d.no_cc where b.id = '$id' GROUP BY a.id ");
    return $hasil->result_array();
}
...
{
    $hasil = $this->db->query("SELECT a.id, a.no_alk, DATE_FORMAT(a.tgl_alk,'%d-%m-%Y') as tgl_alk,a.customer,b.supplier,a.doc_number,d.nama_bank as bank,a.account,a.curr,if(rate = '0', '-',format(a.rate,2)) as rate,if(a.curr = 'IDR',format(a.amount, 2), format(a.amount, 2)) as amount,format(a.eqp_idr, 2) as eqp_idr,a.status from tbl_alokasi AS a INNER JOIN mastersupplier AS b ON a.customer = b.id_supplier left  join masterbank d on d.id = a.bank where a.id = '$id' ");
    return $hasil->row_array();
}
...
{
    $hasil = $this->db->query("insert into tbl_list_journal_cancel (id, no_journal, tgl_journal, type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, faktur_pajak, tgl_faktur_pajak, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, created_at, updated_at, profit_center, supplier) select id, no_journal, tgl_journal, type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, faktur_pajak, tgl_faktur_pajak, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, created_at, updated_at, profit_center, supplier from tbl_list_journal WHERE no_journal = '$id' ");
    return $hasil;
}
...
function cari_kartu_ar($dt_dari_inv, $dt_sampai_inv, $id_customer)
{
    ...
    if ($id_customer == "all_customer") {
     ...
     $hasil = $this->db->query("SELECT * from 
        ...
        (select a.Supplier,round(sum(if(b.shipp = 'Export',c.grand_total ,'0')),2) as totalusd,round(sum(if(b.shipp = 'Local',c.grand_total ,c.grand_total * (select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' AND curr = 'USD' and tanggal = b.tgl_inv))),2) as totalidr from tbl_book_invoice b inner join tbl_invoice_pot c on c.id_book_invoice = b.id inner join mastersupplier a on a.Id_Supplier = b.id_customer where b.status != 'CANCEL' and b.tgl_inv between '$dt_dari_inv' and '$dt_sampai_inv'  GROUP BY supplier) a on ms.namasupplier = a.Supplier
        left join

        (select customer,round(sum(if(curr = 'USD',grand_total ,'0')),2) as saldousd,round(sum(if(curr = 'IDR',grand_total ,grand_total * (select ROUND(rate,2) as rate FROM masterrate where v_codecurr = 'HARIAN' AND curr = 'USD' and tanggal = inv_date))),2) as saldoidr from saldoawal_ar where status != 'CANCEL' GROUP BY customer) b on ms.namasupplier = b.customer
        left join
...
function cari_kartu_ar2($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    if ($id_cus == "all_customer") {
     ...
     $hasil = $this->db->query("SELECT no_invoice,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT no_invoice,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT no_invoice,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due from 
        ...
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a
");
...
function cari_mutasi_debit_note($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    if ($id_cus == "all_customer") {
     ...
     $hasil = $this->db->query("SELECT a.*, ifnull(b.duedate_update,duedate) duedate_update from (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,COALESCE(amount1,0) amount1, duedate, no_invoice1,COALESCE(bayar,0) bayar, no_invoice2,COALESCE(bayar2,0) bayar2, COALESCE(amount,0) amount,IF(supplier is null,'-',supplier) supplier,IF(from_curr = 'IDR','1',rate) rate,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due  from 
        ...
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rate) a) a order by no_invoice asc) a LEFT JOIN (select no_invoice, max(duedate_update) duedate_update from tbl_duedate_update_det where status = 'Y' GROUP BY no_invoice) b on b.no_invoice = a.no_invoice
     ");
...
return $hasil->result_array();
    ...
    $hasil = $this->db->query("SELECT a.*, ifnull(b.duedate_update,duedate) duedate_update from (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,COALESCE(amount1,0) amount1, duedate, no_invoice1,COALESCE(bayar,0) bayar, no_invoice2,COALESCE(bayar2,0) bayar2, COALESCE(amount,0) amount,IF(supplier is null,'-',supplier) supplier,IF(from_curr = 'IDR','1',rate) rate,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due  from 
        ...
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rate) a) a) a LEFT JOIN (select no_invoice, max(duedate_update) duedate_update from tbl_duedate_update_det where status = 'Y' GROUP BY no_invoice) b on b.no_invoice = a.no_invoice");
return $hasil->result_array();
}
}
...
function cari_summary_ar($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    if ($id_cus == "all_customer") {
     ...
     $hasil = $this->db->query("SELECT shipp,id_customer,customer, curr, IF(curr = 'USD',sum(total),0) foreign_curr, sum(eqv_idr) eqv_idr, SUM(not_due) not_due, SUM(amt_aging_1) amt_aging_1, SUM(amt_aging_2) amt_aging_2, SUM(amt_aging_3) amt_aging_3, SUM(amt_aging_4) amt_aging_4, SUM(amt_aging_5) amt_aging_5, SUM(amt_aging_6) amt_aging_6, SUM(amt_aging_7) amt_aging_7, SUM(tot_aging) tot_aging, SUM(ready_due) ready_due, SUM(jml_bln1) jml_bln1, SUM(jml_bln2) jml_bln2, SUM(jml_bln3) jml_bln3, SUM(jml_bln4) jml_bln4, SUM(jml_bln5) jml_bln5, SUM(jml_bln6) jml_bln6,SUM(tot_aging2) tot_aging2 from (SELECT customer, no_invoice, inv_date, shipp, duedate, top, curr, rate, sal_awal,tambah,bayar, total, eqv_idr, IF(diff_top <= 0,eqv_idr,0) not_due, IF(diff_top > 0 AND diff_top <= 30,eqv_idr,0) amt_aging_1, IF(diff_top > 30 AND diff_top <= 60,eqv_idr,0) amt_aging_2, IF(diff_top > 60 AND diff_top <= 90,eqv_idr,0) amt_aging_3, IF(diff_top > 90 AND diff_top <= 120,eqv_idr,0) amt_aging_4, IF(diff_top > 120 AND diff_top <= 180,eqv_idr,0) amt_aging_5, IF(diff_top > 180 AND diff_top <= 360,eqv_idr,0) amt_aging_6, IF(diff_top > 360,eqv_idr,0) amt_aging_7,eqv_idr tot_aging,IF(duedate <= '$dt_sampai_alk',eqv_idr,0) ready_due, IF(jml_bln1 > 0 AND duedate > '$dt_sampai_alk',eqv_idr,0) jml_bln1, IF(jml_bln2 > 0,eqv_idr,0) jml_bln2, IF(jml_bln3 > 0,eqv_idr,0) jml_bln3, IF(jml_bln4 > 0,eqv_idr,0) jml_bln4, IF(jml_bln5 > 0,eqv_idr,0) jml_bln5, IF(jml_bln6 > 0,eqv_idr,0) jml_bln6, eqv_idr tot_aging2, id_customer from (SELECT customer, no_invoice, inv_date, shipp, duedate, top, curr, rate, sal_awal,tambah,bayar, ((sal_awal + tambah) - bayar) total, (if(((sal_awal + tambah) - bayar) >= 0.1,((sal_awal + tambah) - bayar),0) * rate) eqv_idr,ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6,diff_top,id_customer  from (SELECT customer, no_invoice, inv_date, shipp, duedate, top, curr, IF(curr = 'USD',rate,'1') rate, IF(inv_date >= '$dt_sampai_alk','0',(COALESCE(amount1,0)) - COALESCE(bayar2,0)) sal_awal, IF(inv_date >= '$dt_sampai_alk',(COALESCE(amount1,0)) - COALESCE(bayar2,0),'0') tambah, bayar,ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, diff_top,id_customer from (SELECT no_invoice,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT no_invoice,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT no_invoice,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due from 
        ...
        (select a.no_ref as no_invoice1, sum(a.amount) as bayar from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk between '$dt_dari_alk' and '$dt_sampai_alk' and a.total != '0' group by a.no_ref) byr on byr.no_invoice1 = inv.no_invoice LEFT JOIN
        (select a.no_ref as no_invoice2, sum(a.amount) as bayar2 from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk < '$dt_dari_alk' and a.total != '0' group by a.no_ref) byr2 on byr2.no_invoice2 = inv.no_invoice JOIN
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a ) a GROUP BY a.id_customer,shipp
");
...
function cari_summary_dn($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    if ($id_cus == "all_customer") {
     ...
     $hasil = $this->db->query("SELECT id_customer,customer, from_curr, IF(from_curr = 'USD',sum(total),0) foreign_curr, sum(COALESCE(eqv_idr,0)) eqv_idr, SUM(not_due) not_due, SUM(amt_aging_1) amt_aging_1, SUM(amt_aging_2) amt_aging_2, SUM(amt_aging_3) amt_aging_3, SUM(amt_aging_4) amt_aging_4, SUM(amt_aging_5) amt_aging_5, SUM(amt_aging_6) amt_aging_6, SUM(amt_aging_7) amt_aging_7, SUM(tot_aging) tot_aging, SUM(ready_due) ready_due, SUM(jml_bln1) jml_bln1, SUM(jml_bln2) jml_bln2, SUM(jml_bln3) jml_bln3, SUM(jml_bln4) jml_bln4, SUM(jml_bln5) jml_bln5, SUM(jml_bln6) jml_bln6,SUM(tot_aging2) tot_aging2 from (SELECT customer, no_invoice, inv_date,  duedate, top, from_curr, rate, sal_awal,tambah,bayar, total, eqv_idr, IF(diff_top <= 0,eqv_idr,0) not_due, IF(diff_top > 0 AND diff_top <= 30,eqv_idr,0) amt_aging_1, IF(diff_top > 30 AND diff_top <= 60,eqv_idr,0) amt_aging_2, IF(diff_top > 60 AND diff_top <= 90,eqv_idr,0) amt_aging_3, IF(diff_top > 90 AND diff_top <= 120,eqv_idr,0) amt_aging_4, IF(diff_top > 120 AND diff_top <= 180,eqv_idr,0) amt_aging_5, IF(diff_top > 180 AND diff_top <= 360,eqv_idr,0) amt_aging_6, IF(diff_top > 360,eqv_idr,0) amt_aging_7,eqv_idr tot_aging,IF(duedate <= '$dt_sampai_alk',eqv_idr,0) ready_due, IF(jml_bln1 > 0 AND duedate > '$dt_sampai_alk',eqv_idr,0) jml_bln1, IF(jml_bln2 > 0,eqv_idr,0) jml_bln2, IF(jml_bln3 > 0,eqv_idr,0) jml_bln3, IF(jml_bln4 > 0,eqv_idr,0) jml_bln4, IF(jml_bln5 > 0,eqv_idr,0) jml_bln5, IF(jml_bln6 > 0,eqv_idr,0) jml_bln6, eqv_idr tot_aging2, id_customer from (SELECT customer, no_invoice, inv_date,  duedate, top, from_curr, rate, sal_awal,tambah,bayar, ((sal_awal + tambah) - bayar) total, (if(((sal_awal + tambah) - bayar) >= 0.2,((sal_awal + tambah) - bayar),0) * rate) eqv_idr,ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6,diff_top,id_customer  from (SELECT customer, no_invoice, inv_date,  duedate, top, from_curr, IF(from_curr = 'USD',rate,'1') rate, IF(inv_date >= '$dt_sampai_alk','0',(COALESCE(amount1,0)) - COALESCE(bayar2,0)) sal_awal, IF(inv_date >= '$dt_sampai_alk',(COALESCE(amount1,0)) - COALESCE(bayar2,0),'0') tambah, bayar,ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, diff_top,id_customer from (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,COALESCE(amount1,0) amount1, duedate, no_invoice1,COALESCE(bayar,0) bayar, no_invoice2,COALESCE(bayar2,0) bayar2, COALESCE(amount,0) amount,IF(supplier is null,'-',supplier) supplier,IF(from_curr = 'IDR','1',rate) rate,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due  from 
        ...
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rate) a) a) a) a) a ) a GROUP BY a.id_customer,from_curr
");
...
function cari_summary_all($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    if ($id_cus == "all_customer") {
     ...
     $hasil = $this->db->query("(SELECT shipp, id_customer, customer, curr, IF(curr = 'USD',sum(total),0) foreign_curr, sum(eqv_idr) eqv_idr , SUM(amt_aging_0) not_due, SUM(amt_aging_1) amt_aging_1, SUM(amt_aging_2) amt_aging_2, SUM(amt_aging_3) amt_aging_3, SUM(amt_aging_4) amt_aging_4, SUM(amt_aging_5) amt_aging_5, SUM(amt_aging_6) amt_aging_6, SUM(amt_aging_7) amt_aging_7, SUM(tot_aging) tot_aging , SUM(readydue) ready_due, SUM(hasil_bln1) jml_bln1, SUM(hasil_bln2) jml_bln2, SUM(hasil_bln3) jml_bln3, SUM(hasil_bln4) jml_bln4, SUM(hasil_bln5) jml_bln5, SUM(hasil_bln6) jml_bln6,SUM(tot_jatem) tot_aging2 from (select  id_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, if(curr = 'IDR', 1, rate) rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) > '$dt_sampai_alk' THEN eqv_idr ELSE 0 END AS hasil_bln1,
        ...
        from (select a.*, ((sal_awl + tambah) - bayar) total, ROUND(IF(curr = 'USD',((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0)) * rate,((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0))),2) eqv_idr from (select no_invoice, kode_customer, customer, inv_date, id_customer, curr, top, amount1, duedate, bayar, bayar2, rate, shipp, diff_top, ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, IF(inv_date >= '$dt_dari_alk',0,COALESCE(amount1,0) - COALESCE(bayar2,0)) sal_awl, IF(inv_date >= '$dt_dari_alk',COALESCE(amount1,0) - COALESCE(bayar2,0),0) tambah from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT profit_center, no_invoice,kode_customer,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,CASE WHEN bayar > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar END bayar,no_invoice2, CASE WHEN bayar2 > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar2 END bayar2,rate,shipp,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due from 
        ...
        where a.sj_date between '2022-05-01' and '$dt_sampai_alk' and a.profit_center = 'NAG'
        UNION
        SELECT distinct a.profit_center, a.no_invoice AS no_invoice, UPPER(b.supplier) AS customer, UPPER(b.supplier_code) kode_customer,a.sj_date inv_date,a.sj_date tgl_inv, b.Id_Supplier AS id_customer, a.curr,f.top,
        FORMAT((d.grand_total), 2) AS amount, if(a.curr = 'IDR',round((d.grand_total),0),round((d.grand_total), 2)) AS amount1,if(h.kontrabon_date is null, DATE_ADD(DATE_FORMAT(a.sj_date, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(h.kontrabon_date, INTERVAL f.top DAY)) AS duedate,a.shipp
        FROM  tbl_book_invoice AS a INNER JOIN 
        ...
        tbl_master_top AS f ON a.id_top = f.id left join 
        tbl_duedate AS h ON a.id = h.id_invoice
        where a.sj_date between '2022-05-01' and '$dt_sampai_alk' and a.profit_center = 'NAK'
        UNION
        ...
        (select a.no_ref as no_invoice1, sum(a.amount) as bayar from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk between '$dt_dari_alk' and '$dt_sampai_alk' and a.total != '0' group by a.no_ref) byr on byr.no_invoice1 = inv.no_invoice LEFT JOIN
        (select a.no_ref as no_invoice2, sum(a.amount) as bayar2 from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk < '$dt_dari_alk' and a.total != '0' group by a.no_ref) byr2 on byr2.no_invoice2 = inv.no_invoice JOIN
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a) a WHERE sal_awl > 0 OR tambah > 0 OR bayar > 0 OR total > 0) a GROUP BY id_customer,shipp) 
...
return $hasil->result_array();
    ...
    $hasil = $this->db->query("(SELECT shipp, id_customer, customer, curr, IF(curr = 'USD',sum(total),0) foreign_curr, sum(eqv_idr) eqv_idr , SUM(amt_aging_0) not_due, SUM(amt_aging_1) amt_aging_1, SUM(amt_aging_2) amt_aging_2, SUM(amt_aging_3) amt_aging_3, SUM(amt_aging_4) amt_aging_4, SUM(amt_aging_5) amt_aging_5, SUM(amt_aging_6) amt_aging_6, SUM(amt_aging_7) amt_aging_7, SUM(tot_aging) tot_aging , SUM(readydue) ready_due, SUM(hasil_bln1) jml_bln1, SUM(hasil_bln2) jml_bln2, SUM(hasil_bln3) jml_bln3, SUM(hasil_bln4) jml_bln4, SUM(hasil_bln5) jml_bln5, SUM(hasil_bln6) jml_bln6,SUM(tot_jatem) tot_aging2 from (select  id_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, if(curr = 'IDR', 1, rate) rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) > '$dt_sampai_alk' THEN eqv_idr ELSE 0 END AS hasil_bln1,
        ...
        (select a.no_ref as no_invoice1, sum(a.amount) as bayar from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk between '$dt_dari_alk' and '$dt_sampai_alk' and a.total != '0' group by a.no_ref) byr on byr.no_invoice1 = inv.no_invoice LEFT JOIN
        (select a.no_ref as no_invoice2, sum(a.amount) as bayar2 from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk < '$dt_dari_alk' and a.total != '0' group by a.no_ref) byr2 on byr2.no_invoice2 = inv.no_invoice JOIN
        (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a) a WHERE sal_awl > 0 OR tambah > 0 OR bayar > 0 OR total > 0) a WHERE id_customer = '$id_cus' GROUP BY id_customer,shipp) 
...
function cari_sls_ytd_inv($filter)
{
    ...
    $query = $this->db->query(" SELECT sum(eqv_idr) sls_ytd_inv from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,total from tbl_book_invoice a inner join tbl_invoice_pot b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAG' and a.sj_date BETWEEN CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y'),'-01-','01') and CURRENT_DATE() ) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date
        UNION
        select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,grand_total total from tbl_book_invoice a inner join tbl_invoice_pot_knitting b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAK' and a.sj_date BETWEEN CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y'),'-01-','01') and CURRENT_DATE() ) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date
        UNION ALL
        select a.customer id_customer,c.Supplier customer,a.no_inv no_invoice,a.inv_date sj_date,a.type id_type,'IDR' curr, 1 rate, total, total eqv_idr from tbl_invoice_nb a inner join tbl_invoice_nb_pot b on b.no_inv = a.no_inv inner join mastersupplier c on c.Id_Supplier = a.customer where a.type = 'Commercial' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.inv_date BETWEEN CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y'),'-01-','01') and CURRENT_DATE()) a");
}
...
function cari_sls_cm_inv($filter)
{
    ...
    $where = "";
    $query = $this->db->query("
        SELECT sum(eqv_idr) sls_cm_inv from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,grand_total total from tbl_book_invoice a inner join tbl_invoice_pot_knitting b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAK' and a.sj_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date
            UNION
            select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,total from tbl_book_invoice a inner join tbl_invoice_pot b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAG' and a.sj_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) 
                UNION 
                select a.customer id_customer,a.status,a.id,c.Supplier customer,a.no_inv no_invoice,a.inv_date sj_date,a.type id_type,'IDR' curr,total from tbl_invoice_nb a inner join tbl_invoice_nb_pot b on b.no_inv = a.no_inv inner join mastersupplier c on c.Id_Supplier = a.customer where a.type = 'Commercial' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.inv_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date) a");
}
        // $query = $this->db->query("SELECT sum(eqv_idr) sls_cm_inv from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,total from tbl_book_invoice a inner join tbl_invoice_pot b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.sj_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date) a");
...
//     $hasil = $db_pgsql->query("SELECT * from (SELECT f.so_date, a.id, f.jenis_order jns_so, f.tipe_order_import_export area, mk.nama_konsumen, mk.kode_konsumen, kode_so no_so, kode_out sj, tgl_pengeluaran bppbdate, kode_out shipping_number, '-' ws, lab_dip styleno, '-' product_group, nama_kain product_item, warna color, '-' size,  currency curr, g.nama_unit uom, qty_meter qty, Round(coalesce(harga_shipment,0),4) AS unit_price, ROUND(qty_meter * Round(coalesce(harga_shipment,0),4), 4) AS total_price, a.no_so id_so, a.id AS id_bppb, 'A' grade, '' not_sales, CONCAT(ROUND(qty_meter,2),' ',g.nama_unit) qty_pcs, TO_CHAR(ROUND(qty_meter * ROUND(COALESCE(harga_shipment,0),4), 2), 'FM999,999,999,990.00') AS total, '-' invno from official_out_h a 
...
//         where a.status_inv is null and a.tipe_pengeluaran IN ('Penjualan', 'Pengiriman Retur perbaikan ke Subkontraktor Jasa') and tgl_pengeluaran BETWEEN '$dt_dari_sj' AND '$dt_sampai_sj' ORDER BY kode_out asc) a");
...
function cari_sj_noncom($dt_dari_sj, $dt_sampai_sj)
{
    $db_nag = $this->load->database('db_nag', TRUE); // MySQL
    $db_pgsql = $this->load->database('db_pgsql', TRUE); // PostgreSQL

    $rate_rows = $db_nag->query("SELECT tanggal, rate FROM masterrate WHERE tanggal BETWEEN '$dt_dari_sj' AND '$dt_sampai_sj' AND v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal")->result_array();

    $bppb_update = $db_nag->query("SELECT bppbno_int FROM bppb WHERE bppbdate BETWEEN '$dt_dari_sj' AND '$dt_sampai_sj' AND bppbno_int like '%OFC%' and not_sales is not null GROUP BY bppbno_int")->result_array();

    $bppb_list = array_column($bppb_update, 'bppbno_int');

    $rate_map = [];
    foreach ($rate_rows as $row) {
        $rate_map[$row['tanggal']] = $row['rate'];
    }

    $hasil_mysql = $db_nag->query("SELECT *,IF(curr = 'USD',rate,1) rates, round((total_price * IF(curr = 'USD',rate,1)),2) ttl_price, round((total_price_ship * IF(curr = 'USD',rate,1)),2) ttl_price_ship, FORMAT(round((total_price * IF(curr = 'USD',rate,1)),2),2) total_price2, FORMAT(round((total_price * IF(curr = 'USD',rate,1)),2),2) total_price2_ship from (SELECT a.so_date, c.id, jns_so,if(f.area = 'I','Export','Local') area,f.supplier,f.id_supplier,a.so_no AS no_so, c.bppbno AS sj, c.bppbdate, c.bppbno_int AS shipping_number, d.kpno AS ws,  
        d.styleno, e.product_group, e.product_item, b.color, b.size,  
        c.curr, c.unit AS uom, c.qty, format(Round(b.price,4),2) AS unit_price,  
        ROUND(c.qty * Round(b.price,4), 4) AS total_price,  b.id_so, c.id AS id_bppb,if(c.grade = 'GRADE A','A','B') as grade,c.not_sales , CONCAT(c.qty,' ',c.unit) qty_pcs, format(ROUND(c.qty * Round(b.price,4), 4),2) total, c.invno, c.unit AS uom_ship, c.qty qty_ship, format(Round(b.price,4),2) AS unit_price_ship,  
        ROUND(c.qty * Round(b.price,4), 4) AS total_price_ship, format(ROUND(c.qty * Round(b.price,4), 4),2) total_ship
        FROM so AS a INNER JOIN 
        so_det AS b ON a.id = b.id_so INNER JOIN 
        bppb AS c ON b.id = c.id_so_det INNER JOIN 
        act_costing AS d ON a.id_cost = d.id INNER JOIN 
    ...
    $hasil_pgsql = $db_pgsql->query("SELECT * from (SELECT f.so_date, a.id, f.jenis_order jns_so, f.tipe_order_import_export area, mk.nama_konsumen supplier, mk.kode_konsumen id_supplier, kode_so no_so, kode_out sj, tgl_pengeluaran bppbdate, kode_out shipping_number, '-' ws, lab_dip styleno, '-' product_group, nama_kain product_item, warna color, '-' size,  currency curr, h.nama_unit uom, qty_netto qty, Round(coalesce(harga,0),4) AS unit_price, ROUND(qty_netto * Round(coalesce(harga,0),4), 4) AS total_price, a.no_so id_so, a.id AS id_bppb, 'A' grade, '' not_sales, CONCAT(ROUND(qty_netto,2),' ',h.nama_unit) qty_pcs, TO_CHAR(ROUND(qty_netto * ROUND(coalesce(harga,0),4), 2), 'FM999,999,999,990.00') AS total, '-' invno, h.nama_unit uom_ship, qty_netto qty_ship, Round(coalesce(harga,0),4) AS unit_price_ship, ROUND(qty_netto * Round(coalesce(harga,0),4), 4) AS total_price_ship, TO_CHAR(ROUND(qty_netto * ROUND(coalesce(harga,0),4), 2), 'FM999,999,999,990.00') AS total_ship from official_out_h a
        ...
        left join master_unit h on h.id = e.id_unit_sales_order
        INNER JOIN master_konsumen mk on mk.id = f.konsumen_id
        where a.status_inv is null and a.tipe_pengeluaran IN ('Penjualan') and tgl_pengeluaran BETWEEN '$dt_dari_sj' AND '$dt_sampai_sj' ORDER BY kode_out asc) a")->result_array();

    $hasil_pgsql = array_filter($hasil_pgsql, function($row) use ($bppb_list) {
        return !in_array($row['shipping_number'], $bppb_list);
    });


    foreach ($hasil_pgsql as &$row) {
        $tanggal = $row['bppbdate'];
        $rate = isset($rate_map[$tanggal]) ? $rate_map[$tanggal] : 1;
        $row['rate'] = ($row['curr'] == 'USD') ? $rate : 1;

        $row['ttl_price'] = round($row['total_price'] * $row['rate'], 2);
        $row['total_price2'] = number_format($row['ttl_price'], 2, '.', ',');
        $row['ttl_price_ship'] = round($row['total_price_ship'] * $row['rate'], 2);
        $row['total_price2_ship'] = number_format($row['ttl_price_ship'], 2, '.', ',');
    }

    $hasil = array_merge($hasil_mysql, $hasil_pgsql);
...
{
    $hasil = $this->db->query("UPDATE bppb SET not_sales = 'Y' WHERE bppbno_int = '$id' ");
    return $hasil;
}
...
function modal_caridata_slsytd($filter)
{
    ...
    $where = "";
    $hasil = $this->db->query("SELECT * from (SELECT customer, qty,CONCAT(if(uom='PCS',format(qty,0),format(qty,2)),' ',uom) qty2, total, CONCAT('IDR ',format(total,2)) total2 from (SELECT customer,sum(qty) qty, sum(eqv_idr) total, uom from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr,qty, uom from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,total,uom from tbl_book_invoice a inner join tbl_invoice_pot b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer
       INNER JOIN (select id_book_invoice, uom from tbl_invoice_detail where id_book_invoice is not null GROUP BY id_book_invoice) di on di.id_book_invoice = a.id where a.id_type = '1' and a.status IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAG' and a.sj_date BETWEEN CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y'),'-01-','01') and CURRENT_DATE() ) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date LEFT JOIN
    (select id_book_invoice,sum(qty) qty from tbl_invoice_detail where id_book_invoice is not null GROUP BY id_book_invoice) c on c.id_book_invoice = a.id) a GROUP BY customer) a order by qty desc) a
    UNION ALL
    select c.Supplier customer, qty, CONCAT(if(uom='PCS',format(qty,0),format(qty,2)),' ',uom) qty2, total, CONCAT('IDR ',format(total,2)) total2 from tbl_invoice_nb a inner join tbl_invoice_nb_pot b on b.no_inv = a.no_inv inner join mastersupplier c on c.Id_Supplier = a.customer INNER JOIN (select no_inv, sum(qty) qty, uom from tbl_invoice_nb_detail GROUP BY no_inv) d on d.no_inv = a.no_inv where a.type = 'Commercial' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.inv_date BETWEEN CONCAT(DATE_FORMAT(CURRENT_DATE,'%Y'),'-01-','01') and CURRENT_DATE()
    UNION
    SELECT customer, qty,CONCAT(if(uom='PCS',format(qty,0),format(qty,2)),' ',CONVERT(uom USING utf8mb4)) qty2, total, CONCAT('IDR ',format(total,2)) total2 from (SELECT customer,sum(qty) qty, sum(eqv_idr) total, uom from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr,qty, uom from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,grand_total total,uom from tbl_book_invoice a inner join tbl_invoice_pot_knitting b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer
...
function modal_caridata_slscm($filter)
{
    ...
    $where = "and a.profit_center = 'NAK'";
    $hasil = $this->db->query("SELECT customer, qty,CONCAT(FORMAT(qty, 0), ' ', CONVERT(uom USING utf8mb4)) qty2, total, CONCAT('IDR ',format(total,2)) total2 from (SELECT customer,sum(qty) qty, sum(eqv_idr) total,uom from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr,qty, c.uom from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,grand_total total from tbl_book_invoice a inner join tbl_invoice_pot_knitting b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') $where and a.sj_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date LEFT JOIN (select id_book_invoice,sum(qty) qty, uom from tbl_invoice_detail_knitting where id_book_invoice is not null GROUP BY id_book_invoice) c on c.id_book_invoice = a.id) a GROUP BY customer) a order by qty desc");
}else{
    $hasil = $this->db->query(" SELECT * from (SELECT customer, qty,CONCAT(format(qty,0),' PCS') qty2, total, CONCAT('IDR ',format(total,2)) total2 from (SELECT customer,sum(qty) qty, sum(eqv_idr) total from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr,qty from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,total from tbl_book_invoice a inner join tbl_invoice_pot b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAG' and a.sj_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date LEFT JOIN (select id_book_invoice,sum(qty) qty from tbl_invoice_detail where id_book_invoice is not null GROUP BY id_book_invoice) c on c.id_book_invoice = a.id) a GROUP BY customer) a order by qty desc) a
        UNION ALL
        select c.Supplier customer, qty, CONCAT(if(uom='PCS',format(qty,0),format(qty,2)),' ',uom) qty2, total, CONCAT('IDR ',format(total,2)) total2 from tbl_invoice_nb a inner join tbl_invoice_nb_pot b on b.no_inv = a.no_inv inner join mastersupplier c on c.Id_Supplier = a.customer INNER JOIN (select no_inv, sum(qty) qty, uom from tbl_invoice_nb_detail GROUP BY no_inv) d on d.no_inv = a.no_inv where a.type = 'Commercial' and a.status  IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.inv_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))
        UNION
        SELECT customer, qty,CONCAT(FORMAT(qty, 0), ' ', CONVERT(uom USING utf8mb4)) qty2, total, CONCAT('IDR ',format(total,2)) total2 from (SELECT customer,sum(qty) qty, sum(eqv_idr) total,uom from (select id_customer,customer,no_invoice,sj_date,id_type,curr,IF(curr = 'USD',rate,1) rate,total, round((total * IF(curr = 'USD',rate,1)),2) eqv_idr,qty, c.uom from (select a.id_customer,a.status,a.id,c.Supplier customer,a.no_invoice,a.sj_date,a.id_type,a.curr,grand_total total from tbl_book_invoice a inner join tbl_invoice_pot_knitting b on b.id_book_invoice = a.id inner join mastersupplier c on c.Id_Supplier = a.id_customer where a.id_type = '1' and a.status IN ('APPROVED','POST','FIRST APPROVED','SECOND APPROVED') and a.profit_center = 'NAK' and a.sj_date BETWEEN (select min(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y')) and (select max(tanggal) from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE,'%m') and tahun = DATE_FORMAT(CURRENT_DATE,'%Y'))) a left JOIN (select tanggal,rate from masterrate where v_codecurr = 'PAJAK' AND curr = 'USD' GROUP BY tanggal) b on b.tanggal = a.sj_date LEFT JOIN (select id_book_invoice,sum(qty) qty, uom from tbl_invoice_detail_knitting where id_book_invoice is not null GROUP BY id_book_invoice) c on c.id_book_invoice = a.id) a GROUP BY customer) a order by qty desc");
...
function cari_data_doc_reverse($dt_dari_doc, $dt_sampai_doc, $id_customer, $doc_type)
{
    ...
    if ($doc_type == 'INV') {
        ...
        $hasil = $this->db->query("select a.no_kwt no_document, tgl_kwt tgl_document, ms.supplier customer, 'IDR' curr, a.total, a.supp id_customer from tbl_kwitansi a INNER JOIN mastersupplier ms on ms.id_supplier = a.supp where tgl_kwt >= '$dt_dari_doc' AND tgl_kwt <= '$dt_sampai_doc' ".$where." AND a.status like '%APPROVED%' group by a.no_kwt");
...
function approve_doc_reverse($id)
{
    ...
    $query = $this->db->query("SELECT rvs_number FROM tbl_reverse_h WHERE id = '$id'");
    $row = $query->row();
    $rvs_number = $row->rvs_number;

    $update_dn = $this->db->query("UPDATE tbl_debitnote_h a
    ...
    $get_reff = $this->db->query("
        SELECT DISTINCT doc_number 
        FROM tbl_reverse_det 
        WHERE rvs_number = '$rvs_number'
        ")->result();

    foreach ($get_reff as $reff) {
        $no_reff = $reff->doc_number;

        $update_journal = $this->db->query("INSERT into tbl_list_journal (no_journal, tgl_journal, type_journal, no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, faktur_pajak, tgl_faktur_pajak, buyer, no_ws, curr, rate, debit, credit, debit_idr, credit_idr, status, keterangan, create_by, create_date, approve_by, approve_date, cancel_by, cancel_date, created_at, updated_at, profit_center, supplier) select no_journal, tgl_journal, CONCAT('Reverse ',type_journal), no_coa, nama_coa, no_costcenter, nama_costcenter, reff_doc, reff_date, faktur_pajak, tgl_faktur_pajak, buyer, no_ws, curr, rate, credit, debit, credit_idr, debit_idr, status, keterangan, create_by, create_date, '' approve_by, CURRENT_TIMESTAMP() approve_date, cancel_by, cancel_date, created_at, updated_at, profit_center, supplier from tbl_list_journal where no_journal = '$no_reff'");
    }
...
{
    $hasil = $this->db->query("SELECT id_bppb, so_number, bppb_number, sj_date, shipp_number, ws, styleno, product_group, product_item, color, size, curr, uom, qty, unit_price, disc, total_price, uom_ship, qty_ship, unit_price_ship, total_price_ship, po_konsumen, id_konsumen FROM tbl_invoice_detail_knitting_temp");
    return $hasil->result_array();
}
...
{
    $hasil = $this->db->query("(SELECT styleno, product_group, product_item, color, size, qty, format(round(unit_price,3),3) unit_price, disc, FORMAT(total_price, 2) AS total_price, uom, curr, id_bppb,po_konsumen
       FROM tbl_invoice_detail_knitting
       WHERE id_book_invoice = '$id' ORDER BY id_bppb asc)
       UNION
       (select c.styleno, c.product_group, nama_pilihan product_item, c.color, '' size, a.qty, format(round(a.price,3),3) unit_price, 0 disc, FORMAT(a.total, 2) AS total_price, c.uom, c.curr, c.id_bppb,GROUP_CONCAT(DISTINCT po_konsumen SEPARATOR ', ') po_konsumen from tbl_invoice_other_charge a INNER JOIN tbl_pilihan_ar b on b.id = a.id_ctg INNER JOIN tbl_invoice_detail_knitting c on c.id_book_invoice = a.id_book_invoice WHERE a.id_book_invoice = '$id' group by a.id) ");
    return $hasil->result_array();
}
...
function get_konsumen_invoice($id)
{
    $hasil = $this->db->query("SELECT supplier, alamat from tbl_invoice_detail_knitting a INNER JOIN mastersupplier b on b.knitting_code = a.id_konsumen where id_book_invoice = '$id' limit 1 ");
    return $hasil->row_array();
}

function group_curr_knitting($id)
{
    $hasil = $this->db->query("SELECT DISTINCT curr, uom
       FROM tbl_invoice_detail_knitting
       WHERE id_book_invoice = '$id' ");
    return $hasil->row_array();
}
...
function cancel_doc_reverse($id)
{
    $hasil = $this->db->query("UPDATE tbl_reverse_h SET status = 'CANCEL' WHERE id = '$id' ");

    $query = $this->db->query("SELECT rvs_number FROM tbl_reverse_h WHERE id = '$id'");
    $row = $query->row();
    $rvs_number = $row->rvs_number;

    $update_det = $this->db->query("UPDATE tbl_reverse_det SET status = 'N' WHERE rvs_number = '$rvs_number' ");
...
function cari_kartu_ar_new($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    $hasil = $this->db->query("SELECT a.*, ifnull(b.duedate_update,duedate) duedate_update from (select  id_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, if(curr = 'IDR', 1, rate) rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) >= '$dt_sampai_alk' THEN eqv_idr ELSE 0 END AS hasil_bln1,
        CASE WHEN jml_bln2 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln2,
        CASE WHEN jml_bln3 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln3,
        CASE WHEN jml_bln4 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln4,
        CASE WHEN jml_bln5 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln5,
        ...
        from (select a.*, ((sal_awl + tambah) - bayar) total, ROUND(IF(curr = 'USD',((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0)) * rate,((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0))),2) eqv_idr from (select no_invoice, kode_customer, customer, inv_date, id_customer, curr, top, amount1, duedate, bayar, bayar2, rate, shipp, diff_top, ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, IF(inv_date >= '$dt_dari_alk',0,COALESCE(amount1,0) - COALESCE(bayar2,0)) sal_awl, IF(inv_date >= '$dt_dari_alk',COALESCE(amount1,0) - COALESCE(bayar2,0),0) tambah from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT profit_center, no_invoice,kode_customer,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,CASE WHEN bayar > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar END bayar,no_invoice2, CASE WHEN bayar2 > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar2 END bayar2,rate,shipp,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due from 
            ...
            (select a.no_ref as no_invoice1, sum(a.amount) as bayar from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk between '$dt_dari_alk' and '$dt_sampai_alk' and a.total != '0' group by a.no_ref) byr on byr.no_invoice1 = inv.no_invoice LEFT JOIN
            (select a.no_ref as no_invoice2, sum(a.amount) as bayar2 from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk < '$dt_dari_alk' and a.total != '0' group by a.no_ref) byr2 on byr2.no_invoice2 = inv.no_invoice JOIN
            (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a) a WHERE sal_awl > 0 OR tambah > 0 OR bayar > 0 OR total > 0) a LEFT JOIN (select no_invoice, max(duedate_update) duedate_update from tbl_duedate_update_det where status = 'Y' GROUP BY no_invoice) b on b.no_invoice = a.no_invoice $where");
...
function cari_summary_ar_new($dt_dari_alk, $dt_sampai_alk, $id_cus)
{
    ...
    $hasil = $this->db->query("SELECT shipp, id_customer, customer, curr, IF(curr = 'USD',sum(total),0) foreign_curr, sum(eqv_idr) eqv_idr , SUM(amt_aging_0) not_due, SUM(amt_aging_1) amt_aging_1, SUM(amt_aging_2) amt_aging_2, SUM(amt_aging_3) amt_aging_3, SUM(amt_aging_4) amt_aging_4, SUM(amt_aging_5) amt_aging_5, SUM(amt_aging_6) amt_aging_6, SUM(amt_aging_7) amt_aging_7, SUM(tot_aging) tot_aging , SUM(readydue) ready_due, SUM(hasil_bln1) jml_bln1, SUM(hasil_bln2) jml_bln2, SUM(hasil_bln3) jml_bln3, SUM(hasil_bln4) jml_bln4, SUM(hasil_bln5) jml_bln5, SUM(hasil_bln6) jml_bln6,SUM(tot_jatem) tot_aging2 from (select  id_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, if(curr = 'IDR', 1, rate) rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) > '$dt_sampai_alk' THEN eqv_idr ELSE 0 END AS hasil_bln1,
        ...
        from (select a.*, ((sal_awl + tambah) - bayar) total, ROUND(IF(curr = 'USD',((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0)) * rate,((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0))),2) eqv_idr from (select no_invoice, kode_customer, customer, inv_date, id_customer, curr, top, amount1, duedate, bayar, bayar2, rate, shipp, diff_top, ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, IF(inv_date >= '$dt_dari_alk',0,COALESCE(amount1,0) - COALESCE(bayar2,0)) sal_awl, IF(inv_date >= '$dt_dari_alk',COALESCE(amount1,0) - COALESCE(bayar2,0),0) tambah from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT profit_center, no_invoice,kode_customer,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,CASE WHEN bayar > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar END bayar,no_invoice2, CASE WHEN bayar2 > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar2 END bayar2,rate,shipp,DATEDIFF('$dt_sampai_alk',duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT('$dt_sampai_alk','%m') fil_bln1,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT('$dt_sampai_alk','%m') + 5,2,0) fil_bln6, DATE_FORMAT('$dt_sampai_alk','%Y') fil_thn, IF(duedate <= '$dt_sampai_alk',amount1,0) ready_due from 
            ...
            (select a.no_ref as no_invoice1, sum(a.amount) as bayar from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk between '$dt_dari_alk' and '$dt_sampai_alk' and a.total != '0' group by a.no_ref) byr on byr.no_invoice1 = inv.no_invoice LEFT JOIN
            (select a.no_ref as no_invoice2, sum(a.amount) as bayar2 from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk < '$dt_dari_alk' and a.total != '0' group by a.no_ref) byr2 on byr2.no_invoice2 = inv.no_invoice JOIN
            (select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a) a WHERE sal_awl > 0 OR tambah > 0 OR bayar > 0 OR total > 0) a $where GROUP BY id_customer,shipp");

return $hasil->result_array();

}

function get_pilihan_duedate_update()
{
    $hasil = $this->db->query("SELECT id, kode_pilihan, nama_pilihan FROM tbl_pilihan_ar where ctg_pilihan = 'duedate update' ");
    return $hasil->result_array();
}
...
function cari_data_reff_duedate($dt_dari_doc, $dt_sampai_doc, $id_customer, $doc_type)
{
    ...
    if ($doc_type == 'INV') {
        ...
        $hasil = $this->db->query("select no_invoice, id_customer, customer, inv_date, duedate, top, curr, rate, total, eqv_idr from (select  id_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, if(curr = 'IDR', 1, rate) rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) >= CURRENT_DATE() THEN eqv_idr ELSE 0 END AS hasil_bln1,
        CASE WHEN jml_bln2 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln2,
        CASE WHEN jml_bln3 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln3,
        CASE WHEN jml_bln4 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln4,
        CASE WHEN jml_bln5 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln5,
        ...
        from (select a.*, ((sal_awl + tambah) - bayar) total, ROUND(IF(curr = 'USD',((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0)) * rate,((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0))),2) eqv_idr from (select no_invoice, kode_customer, customer, inv_date, id_customer, curr, top, amount1, duedate, bayar, bayar2, rate, shipp, diff_top, ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, IF(inv_date >= CURRENT_DATE(),0,COALESCE(amount1,0) - COALESCE(bayar2,0)) sal_awl, IF(inv_date >= CURRENT_DATE(),COALESCE(amount1,0) - COALESCE(bayar2,0),0) tambah from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT profit_center, no_invoice,kode_customer,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,CASE WHEN bayar > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar END bayar,no_invoice2, CASE WHEN bayar2 > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar2 END bayar2,rate,shipp,DATEDIFF(CURRENT_DATE(),duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT(CURRENT_DATE(),'%m') fil_bln1,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 5,2,0) fil_bln6, DATE_FORMAT(CURRENT_DATE(),'%Y') fil_thn, IF(duedate <= CURRENT_DATE(),amount1,0) ready_due from 
            ...
            (select IF((select id from tbl_tgl_tb where tgl_akhir = CURRENT_DATE()) != '',(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a) a WHERE sal_awl > 0 OR tambah > 0 OR bayar > 0 OR total > 0) a where inv_date >= '$dt_dari_doc' AND inv_date <= '$dt_sampai_doc' ".$where." ");
        ...
        $hasil = $this->db->query("select no_invoice, id_customer, customer, inv_date, duedate, top, from_curr curr, rate, ((amount1 - IFNULL(bayar2,0)) - IFNULL(bayar,0)) AS total, ((amount1 - IFNULL(bayar2,0)) - IFNULL(bayar,0)) * IFNULL(rate,1) AS eqv_idr from (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,COALESCE(amount1,0) amount1, duedate, no_invoice1,COALESCE(bayar,0) bayar, no_invoice2,COALESCE(bayar2,0) bayar2, COALESCE(amount,0) amount,IF(supplier is null,'-',supplier) supplier,IF(from_curr = 'IDR','1',rate) rate,DATEDIFF(CURRENT_DATE(),duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT(CURRENT_DATE(),'%m') fil_bln1,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 5,2,0) fil_bln6, DATE_FORMAT(CURRENT_DATE(),'%Y') fil_thn, IF(duedate <= CURRENT_DATE(),amount1,0) ready_due  from 
        (
            SELECT distinct a.no_dn AS no_invoice, UPPER(b.supplier) AS customer, a.attn,DATE_FORMAT(a.tgl_dn, '%Y-%m-%d') AS inv_date,DATE_FORMAT(a.tgl_dn, '%Y-%m-%d') AS tgl_inv, b.Id_Supplier AS id_customer, a.to_curr as from_curr,DATEDIFF(a.due_date,a.tgl_dn) top,
            FORMAT(a.eqv_curr, 2) AS amount, if(a.to_curr = 'USD',a.eqv_curr,round(a.eqv_curr, 2)) AS amount1,a.due_date duedate
            FROM  tbl_debitnote_h AS a INNER JOIN 
        ...
        (select IF((select id from tbl_tgl_tb where tgl_akhir = CURRENT_DATE()) != '',(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rate) a) a order by no_invoice asc) a WHERE ((amount1 - IFNULL(bayar2,0)) - IFNULL(bayar,0)) > 0 and inv_date >= '$dt_dari_doc' AND inv_date <= '$dt_sampai_doc' ".$where." ");
        ...
        $hasil = $this->db->query("select * from (select no_invoice, id_customer, customer, inv_date, duedate, top, curr, rate, total, eqv_idr from (select  id_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, if(curr = 'IDR', 1, rate) rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) >= CURRENT_DATE() THEN eqv_idr ELSE 0 END AS hasil_bln1,
        CASE WHEN jml_bln2 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln2,
        CASE WHEN jml_bln3 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln3,
        CASE WHEN jml_bln4 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln4,
        CASE WHEN jml_bln5 > 0 THEN eqv_idr ELSE 0 END AS hasil_bln5,
        ...
        from (select a.*, ((sal_awl + tambah) - bayar) total, ROUND(IF(curr = 'USD',((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0)) * rate,((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0))),2) eqv_idr from (select no_invoice, kode_customer, customer, inv_date, id_customer, curr, top, amount1, duedate, bayar, bayar2, rate, shipp, diff_top, ready_due, jml_bln1, jml_bln2, jml_bln3, jml_bln4, jml_bln5, jml_bln6, IF(inv_date >= CURRENT_DATE(),0,COALESCE(amount1,0) - COALESCE(bayar2,0)) sal_awl, IF(inv_date >= CURRENT_DATE(),COALESCE(amount1,0) - COALESCE(bayar2,0),0) tambah from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 from (SELECT profit_center, no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due from (SELECT profit_center, no_invoice,kode_customer,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,CASE WHEN bayar > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar END bayar,no_invoice2, CASE WHEN bayar2 > 0 AND profit_center = 'NAK' THEN amount1 ELSE bayar2 END bayar2,rate,shipp,DATEDIFF(CURRENT_DATE(),duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT(CURRENT_DATE(),'%m') fil_bln1,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 5,2,0) fil_bln6, DATE_FORMAT(CURRENT_DATE(),'%Y') fil_thn, IF(duedate <= CURRENT_DATE(),amount1,0) ready_due from 
            ...
            (select a.no_ref as no_invoice1, sum(a.amount) as bayar from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk between CURRENT_DATE() and CURRENT_DATE() and a.total != '0' group by a.no_ref) byr on byr.no_invoice1 = inv.no_invoice LEFT JOIN
            (select a.no_ref as no_invoice2, sum(a.amount) as bayar2 from tbl_alokasi_detail a inner join tbl_alokasi b on b.no_alk = a.no_alk where a.status != 'CANCEL' and b.tgl_alk < CURRENT_DATE() and a.total != '0' group by a.no_ref) byr2 on byr2.no_invoice2 = inv.no_invoice JOIN
            (select IF((select id from tbl_tgl_tb where tgl_akhir = CURRENT_DATE()) != '',(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rt) a) a) a) a) a) a WHERE sal_awl > 0 OR tambah > 0 OR bayar > 0 OR total > 0) a where inv_date >= '$dt_dari_doc' AND inv_date <= '$dt_sampai_doc'
                        UNION
select no_invoice, id_customer, customer, inv_date, duedate, top, from_curr curr, rate, ((amount1 - IFNULL(bayar2,0)) - IFNULL(bayar,0)) AS total, ((amount1 - IFNULL(bayar2,0)) - IFNULL(bayar,0)) * IFNULL(rate,1) AS eqv_idr from (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top,ready_due, IF(bln_due = fil_bln1 and thn_due = fil_thn1,amount1,'0') jml_bln1,IF(bln_due = fil_bln2 and thn_due = fil_thn2,amount1,'0') jml_bln2,IF(bln_due = fil_bln3 and thn_due = fil_thn3,amount1,'0') jml_bln3,IF(bln_due = fil_bln4 and thn_due = fil_thn4,amount1,'0') jml_bln4,IF(bln_due = fil_bln5 and thn_due = fil_thn5,amount1,'0') jml_bln5,IF(bln_due = fil_bln6 and thn_due = fil_thn6,amount1,'0') jml_bln6 FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,amount1, duedate, no_invoice1 , bayar, no_invoice2, bayar2, amount, supplier, rate, diff_top, bln_due, thn_due, LPAD(IF(fil_bln1 <= 12,fil_bln1,(fil_bln1 - 12)),2,0) fil_bln1,LPAD(IF(fil_bln2 <= 12,fil_bln2,(fil_bln2 - 12)),2,0) fil_bln2,LPAD(IF(fil_bln3 <= 12,fil_bln3,(fil_bln3 - 12)),2,0) fil_bln3,LPAD(IF(fil_bln4 <= 12,fil_bln4,(fil_bln4 - 12)),2,0) fil_bln4,LPAD(IF(fil_bln5 <= 12,fil_bln5,(fil_bln5 - 12)),2,0) fil_bln5, LPAD(IF(fil_bln6 <= 12,fil_bln6,(fil_bln6 - 12)),2,0) fil_bln6,LPAD(IF(fil_bln1 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn1,LPAD(IF(fil_bln2 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn2,LPAD(IF(fil_bln3 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn3, LPAD(IF(fil_bln4 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn4,LPAD(IF(fil_bln5 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn5, LPAD(IF(fil_bln6 <= 12,fil_thn,(fil_thn + 1)),4,0) fil_thn6, ready_due FROM (SELECT no_invoice,customer,attn,inv_date,tgl_inv,id_customer,from_curr,top,COALESCE(amount1,0) amount1, duedate, no_invoice1,COALESCE(bayar,0) bayar, no_invoice2,COALESCE(bayar2,0) bayar2, COALESCE(amount,0) amount,IF(supplier is null,'-',supplier) supplier,IF(from_curr = 'IDR','1',rate) rate,DATEDIFF(CURRENT_DATE(),duedate) diff_top, DATE_FORMAT(duedate,'%m') bln_due, DATE_FORMAT(duedate,'%Y') thn_due,DATE_FORMAT(CURRENT_DATE(),'%m') fil_bln1,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 1,2,0) fil_bln2, LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 2,2,0) fil_bln3,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 3,2,0) fil_bln4,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 4,2,0) fil_bln5,LPAD(DATE_FORMAT(CURRENT_DATE(),'%m') + 5,2,0) fil_bln6, DATE_FORMAT(CURRENT_DATE(),'%Y') fil_thn, IF(duedate <= CURRENT_DATE(),amount1,0) ready_due  from 
        (
            SELECT distinct a.no_dn AS no_invoice, UPPER(b.supplier) AS customer, a.attn,DATE_FORMAT(a.tgl_dn, '%Y-%m-%d') AS inv_date,DATE_FORMAT(a.tgl_dn, '%Y-%m-%d') AS tgl_inv, b.Id_Supplier AS id_customer, a.to_curr as from_curr,DATEDIFF(a.due_date,a.tgl_dn) top,
            FORMAT(a.eqv_curr, 2) AS amount, if(a.to_curr = 'USD',a.eqv_curr,round(a.eqv_curr, 2)) AS amount1,a.due_date duedate
            FROM  tbl_debitnote_h AS a INNER JOIN 
        ...
        (select IF((select id from tbl_tgl_tb where tgl_akhir = CURRENT_DATE()) != '',(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'HARIAN' AND curr = 'USD'),(select rate from masterrate where tanggal = CURRENT_DATE() and v_codecurr = 'PAJAK' AND curr = 'USD')) rate) rate) a) a order by no_invoice asc) a WHERE ((amount1 - IFNULL(bayar2,0)) - IFNULL(bayar,0)) > 0 and inv_date >= '$dt_dari_doc' AND inv_date <= '$dt_sampai_doc') a where 1=1 ".$where." ");
...
function simpan_duedate_temp($data)
{
    $insert = $this->db->insert_batch('tbl_duedate_update_temp', $data);

    return $insert; // jumlah row yg berhasil
}
...
function delete_data_reff_duedate($user)
{
    ...
    return $this->db->delete('tbl_duedate_update_temp');
}

function load_doc_duedate_temp($user)
{
    $hasil = $this->db->query("select no_dokumen, tgl_dokumen, duedate, id_customer, supplier customer, curr, total_invoice, amount, deskripsi from tbl_duedate_update_temp a INNER JOIN mastersupplier b on b.id_supplier = a.id_customer where a.created_by = '$user'");
    return $hasil->result_array();
}

function get_kode_duedate_update()
{
    $q = $this->db->query("SELECT doc_number, MAX(right(doc_number, 5)) AS kd_max FROM tbl_duedate_update_h WHERE YEAR(created_date) = YEAR(CURRENT_DATE()) AND MONTH(created_date) = MONTH(CURRENT_DATE()) ");
        //
    $kd = "";
    if ($q->num_rows() > 0) {
        foreach ($q->result() as $k) {
    ...
    date_default_timezone_set('Asia/Jakarta');
    return "DDU/" . date('my'). "/" . $kd;
}



function cari_list_duedate_update($dari, $sampai)
{
    if ($dari != 'all' && $sampai != 'all') {
        $this->db->where('DATE(created_date) >=', $dari);
        $this->db->where('DATE(created_date) <=', $sampai);
    }
    ...
    $this->db->select("
        ...
        doc_number,
        duedate_update,
        IF(keterangan = '', '-', keterangan) AS keterangan,
        status,
        CONCAT(created_by,' (',created_date,')') AS user_create
    ");

    $this->db->from('tbl_duedate_update_h');
...
function cancel_duedate_update($id, $doc_number, $user_cancel)
{
    $this->db->trans_start();

    // update header
    $this->db->where('doc_number', $doc_number);
    $this->db->update('tbl_duedate_update_h', [
        'status'      => 'CANCEL',
    ...
    // update detail
    $this->db->where('doc_number', $doc_number); 
    $this->db->update('tbl_duedate_update_det', [
        'status' => 'N'
    ]);
