...
function cari_aging_jatem($id_customer, $start_date, $end_date)
{
    ...
    $hasil = $this->db->query("select kode_customer, '-' id_customer_show, id_customer, customer, top, sum(eqv_idr) total, sum(if(ym_inv_date <= ym_filter6, eqv_idr, 0)) hasil_bln6, sum(if(ym_inv_date = ym_filter5, eqv_idr, 0)) hasil_bln5, sum(if(ym_inv_date = ym_filter4, eqv_idr, 0)) hasil_bln4, sum(if(ym_inv_date = ym_filter3, eqv_idr, 0)) hasil_bln3, sum(if(ym_inv_date = ym_filter2, eqv_idr, 0)) hasil_bln2, sum(if(ym_inv_date = ym_filter0, eqv_idr, 0)) hasil_bln1, sum(if(ym_inv_date = ym_filter0, eqv_idr, 0)) readydue, IF(sum(if(curr != 'IDR',tambah * rate,tambah)) = 0,'No Sales',round(IF(sal_awl = 0 and eqv_idr = 0,'0',(365 / ((sum(if(curr != 'IDR',tambah * rate,tambah)) * 12) / ((sum(if(curr != 'IDR', sal_awl * rate, sal_awl)) + sum(eqv_idr)) / 2)))),2)) ar_day, sum(readydue + hasil_bln1) jatem1, sum(hasil_bln2) jatem31, sum(hasil_bln3) jatem61, sum(hasil_bln4 + hasil_bln5 + hasil_bln6) jatem91 from (select DATE_FORMAT(inv_date,'%y%m') ym_inv_date,DATE_FORMAT(duedate,'%y%m') ym_duedate, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 0 MONTH), '%y%m') AS ym_filter0, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 1 MONTH), '%y%m') ym_filter1, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 2 MONTH), '%y%m') ym_filter2, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 3 MONTH), '%y%m') ym_filter3, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 4 MONTH), '%y%m') ym_filter4, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 5 MONTH), '%y%m') ym_filter5, DATE_FORMAT(DATE_SUB('$end_date', INTERVAL 6 MONTH), '%y%m') ym_filter6, id_customer, kode_customer, customer, no_invoice, inv_date, shipp, duedate, top, curr, rate, sal_awl, tambah, bayar,  total, eqv_idr,amt_aging_0,amt_aging_1,amt_aging_2,amt_aging_3,amt_aging_4,amt_aging_5,amt_aging_6,amt_aging_7, tot_aging, readydue, hasil_bln1, hasil_bln2, hasil_bln3, hasil_bln4, hasil_bln5, hasil_bln6, tot_aging tot_jatem from (select a.*, CASE WHEN jml_bln1 > 0 AND Date(duedate) > '$end_date' THEN eqv_idr ELSE 0 END AS hasil_bln1,
...
function cari_mut_ar($id_customer, $start_date, $end_date)
{
    ...
    $hasil = $this->db->query("select kode_customer, id_customer_show, id_customer, customer, top, sum(sal_awl) sal_awl, sum(tambah) tambah, sum(tambah_ll) tambah_ll, sum(pelunasan) pelunasan, sum(retur) retur, sum(pph_23) pph_23, sum(other) other, sum(sal_akhir) sal_akhir, IF(sum(tambah) = 0,'No Sales',round(IF(sal_awl = 0 and sal_akhir = 0,'0',(365 / ((sum(tambah) * 12) / ((sum(sal_awl) + sum(sal_akhir)) / 2)))),2)) ar_day from (select kode_customer, id_customer_show, id_customer, customer, top, sal_awl, tambah, tambah_ll, pelunasan, retur, pph_23, round(others,2) other, round(sal_awl + (tambah + tambah_ll) - (pelunasan + retur + pph_23 + others),2) sal_akhir from (select kode_customer, '-' id_customer_show, id_customer, customer, top, eqv_idr, sal_awl, tambah, tambah_ll, pelunasan, retur,  - pph_23 pph_23, IF(pelunasan > 0,- ((eqv_idr) - (sal_awl + (tambah + tambah_ll) - (pelunasan + retur + (- pph_23)))),0) others from (select no_invoice, kode_customer, id_customer, customer, top,curr, if (curr = 'IDR', sal_awl, (sal_awl * rate)) sal_awl, if(curr = 'IDR', tambah, (tambah * rate_inv)) tambah, 0 tambah_ll, IF(bayar > 0,if (curr = 'IDR', (total + others), ((total + others) * rate_alk)),0) pelunasan, 0 retur, IF(bayar > 0,if(curr = 'IDR', pph, (pph * rate_alk)),0) pph_23 , eqv_idr from (select a.*, ((sal_awl + tambah) - bayar) total_ending, IF(curr = 'USD',((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0)) * rate_alk,((sal_awl + COALESCE(tambah,0)) - COALESCE(bayar,0))) eqv_idr from (select no_invoice, kode_customer, customer, inv_date, id_customer, curr, top, amount1, duedate, bayar, bayar2, rate, shipp, diff_top, IF(inv_date >= '$start_date',0,COALESCE(amount1,0) - COALESCE(bayar2,0)) sal_awl, IF(inv_date >= '$start_date',COALESCE(amount1,0) - COALESCE(bayar2,0),0) tambah, grand_total, dpp, ppn, pph, total, others, COALESCE(rate_alk,0) rate_alk, IF(curr = 'IDR',1,rate_inv) rate_inv from (SELECT no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,coalesce(bayar,0) bayar,no_invoice2,bayar2,rate,shipp, diff_top, grand_total, dpp, ppn, pph, total, others, rate_alk, rate_inv from (SELECT no_invoice,kode_customer, customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp, diff_top, grand_total, dpp, ppn, pph, total, others, rate_alk, rate_inv from (SELECT no_invoice,kode_customer,customer,inv_date,tgl_inv,id_customer,curr,top,amount,amount1,duedate,no_invoice1,bayar,no_invoice2,bayar2,rate,shipp,DATEDIFF('$end_date',duedate) diff_top, grand_total, dpp, ppn, pph, total, others, rate_alk, rate_inv from 
        (select a.*, round(dpp + ppn_dpp + pph,2) total, round(grand_total - round(dpp + ppn_dpp + pph,2) + pph,2) others from (SELECT distinct a.no_invoice AS no_invoice, UPPER(b.supplier) AS customer, UPPER(b.supplier_code) kode_customer,a.sj_date inv_date,a.sj_date tgl_inv, b.Id_Supplier AS id_customer, a.curr,f.top, FORMAT((d.grand_total), 2) AS amount, round((d.grand_total), 2) AS amount1,if(h.kontrabon_date is null, DATE_ADD(DATE_FORMAT(a.sj_date, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(h.kontrabon_date, INTERVAL f.top DAY)) AS duedate,a.shipp, d.grand_total, d.total AS dpp, d.vat AS ppn, ROUND((d.vat/d.twot) * 100,0) ppn_int, CASE WHEN a.pph = 'PPh 23' THEN - round(d.total * 0.02,2) ELSE 0 END AS pph, d.dp, d.discount, ROUND(IF(d.vat != 0,(d.total * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_dpp, ROUND(IF(d.vat != 0,(d.dp * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_dp, ROUND(IF(d.vat != 0,(d.discount * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_discount
        FROM  tbl_book_invoice AS a INNER JOIN 
        mastersupplier AS b ON a.id_customer = b.id_supplier INNER JOIN 
        tbl_type AS c ON a.id_type = c.id_type INNER JOIN
        tbl_invoice_pot AS d ON a.id = d.id_book_invoice INNER JOIN
        ...
        where a.sj_date between '2022-05-01' and '$end_date' and a.profit_center = 'NAG'
        UNION
        SELECT distinct a.no_invoice AS no_invoice, UPPER(b.supplier) AS customer, UPPER(b.supplier_code) kode_customer,a.sj_date inv_date,a.sj_date tgl_inv, b.Id_Supplier AS id_customer, a.curr,f.top, FORMAT((d.grand_total), 2) AS amount, round((d.grand_total), 2) AS amount1,if(h.kontrabon_date is null, DATE_ADD(DATE_FORMAT(a.sj_date, '%Y-%m-%d'), INTERVAL f.top DAY) ,DATE_ADD(h.kontrabon_date, INTERVAL f.top DAY)) AS duedate,a.shipp, d.grand_total, d.total AS dpp, d.vat AS ppn, ROUND((d.vat/d.twot) * 100,0) ppn_int, CASE WHEN a.pph = 'PPh 23' THEN - round(d.total * 0.02,2) ELSE 0 END AS pph, d.dp, d.discount, ROUND(IF(d.vat != 0,(d.total * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_dpp, ROUND(IF(d.vat != 0,(d.dp * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_dp, ROUND(IF(d.vat != 0,(d.discount * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_discount
        FROM  tbl_book_invoice AS a INNER JOIN 
        mastersupplier AS b ON a.id_customer = b.id_supplier INNER JOIN 
        tbl_type AS c ON a.id_type = c.id_type INNER JOIN
        tbl_invoice_pot_knitting AS d ON a.id = d.id_book_invoice INNER JOIN
        ...
        union
        select a.*, round(dpp + ppn_dpp + pph,2) total, round(grand_total - round(dpp + ppn_dpp + pph,2) + pph,2) others from (SELECT distinct a.no_inv AS no_invoice, UPPER(b.supplier) AS customer, UPPER(b.supplier_code) kode_customer,DATE_FORMAT(e.sj_date, '%Y-%m-%d') AS inv_date,DATE_FORMAT(e.sj_date, '%Y-%m-%d') AS tgl_inv, b.Id_Supplier AS id_customer, e.curr,a.top,
        FORMAT((d.grand_total), 2) AS amount, round((d.grand_total), 2) AS amount1, if(h.kontrabon_date is null, DATE_ADD(DATE_FORMAT(e.sj_date, '%Y-%m-%d'), INTERVAL a.top DAY) ,DATE_ADD(DATE_FORMAT(h.kontrabon_date, '%Y-%m-%d'), INTERVAL a.top DAY)) AS duedate,a.shipp, d.grand_total, d.total AS dpp, d.vat AS ppn, ROUND((d.vat/d.twot) * 100,0) ppn_int, CASE WHEN a.pph = 'PPh 23' THEN - round(d.total * 0.02,2) ELSE 0 END AS pph, d.dp, d.diskon discount, ROUND(IF(d.vat != 0,(d.total * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_dpp, ROUND(IF(d.vat != 0,(d.dp * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_dp, ROUND(IF(d.vat != 0,(d.diskon * (ROUND((d.vat / d.twot) * 100, 0) / 100)),0),2) AS ppn_discount
        FROM  tbl_invoice_nb AS a INNER JOIN 
        mastersupplier AS b ON a.customer = b.id_supplier INNER JOIN 
        tbl_invoice_nb_pot AS d ON a.no_inv = d.no_inv INNER JOIN
        tbl_invoice_nb_detail as e on a.no_inv=e.no_inv left JOIN 
        ...
        union                                                                     
        select a.*, round(dpp + ppn + pph,2) total, round(grand_total - round(dpp + ppn + pph,2) + pph,2) others from (select no_invoice, customer, UPPER(b.supplier_code) kode_customer, inv_date, sj_date as tgl_inv,id_customer, curr, top, FORMAT((grand_total), 2) AS amount, round((grand_total), 2) AS amount1, due_date,shipp, a.grand_total, a.grand_total AS dpp, 0 ppn, 0 ppn_int, 0 pph, 0 dp, 0 discount, 0 ppn_dpp, 0 ppn_dp, 0 ppn_discount from saldoawal_ar a INNER JOIN mastersupplier AS b ON a.id_customer = b.id_supplier where no_invoice not like '%DN/%') a) inv LEFT JOIN
