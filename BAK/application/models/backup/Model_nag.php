...
(select IF((select id from tbl_tgl_tb where tgl_akhir = '$dt_sampai_alk') != '',(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'HARIAN'),(select rate from masterrate where tanggal = '$dt_sampai_alk' and v_codecurr = 'PAJAK')) rate) rate) a) a) a) a) a ) a GROUP BY a.id_customer,from_curr)");
    ...
    {
        $query = $this->db->query("SELECT GROUP_CONCAT(eqv_idr) eqv_idr from (SELECT bulan_text,ROUND(COALESCE(eqv_idr,0),2) eqv_idr FROM (select DISTINCT bulan_text from dim_date WHERE tahun = '2024') a left join 
            (SELECT bulan,sum(eqv_idr) eqv_idr FROM(select * from tbl_data_sales where tahun = '2024') a GROUP BY bulan) b on b.bulan = a.bulan_text ORDER BY a.bulan_text asc) a");
        if ($query->result()) {  
            foreach ($query->result() as $data) {
                $hasil = ($data->eqv_idr);
            }
