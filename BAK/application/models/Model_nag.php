...
{
    $query = $this->db->query("SELECT COALESCE(sum(eqv_idr),0) sales_no_inv from (SELECT bppbdate,curr,total_price,IF(curr = 'USD',rate,1) rate, round((total_price * IF(curr = 'USD',rate,1)),2) eqv_idr from (SELECT a.so_no AS no_so, c.bppbno AS sj, c.bppbdate, c.bppbno_int AS shipping_number, d.kpno AS ws,  
        d.styleno, e.product_group, e.product_item, b.color, b.size,  
...
function cari_sales_ytd_mtm()
{
    $query = $this->db->query("SELECT GROUP_CONCAT(eqv_idr) eqv_idr from (SELECT bulan_text,ROUND(COALESCE(eqv_idr,0),2) eqv_idr FROM (select DISTINCT bulan_text from dim_date WHERE tahun = '2025') a left join 
        (SELECT bulan,sum(eqv_idr) eqv_idr FROM(select * from tbl_data_sales where tahun = '2025') a GROUP BY bulan) b on b.bulan = a.bulan_text ORDER BY a.bulan_text asc) a");
    if ($query->result()) {  
        foreach ($query->result() as $data) {
            $hasil = ($data->eqv_idr);
        }
...
function cari_sales_ytd_mtm_pertahun($tahun)
{
    $query = $this->db->query("SELECT GROUP_CONCAT(eqv_idr) eqv_idr from (SELECT bulan_text,ROUND(COALESCE(eqv_idr,0),2) eqv_idr FROM (select DISTINCT bulan_text from dim_date WHERE tahun = '$tahun') a left join 
        (SELECT bulan,sum(eqv_idr) eqv_idr FROM(select * from tbl_data_sales_$tahun where tahun = '$tahun') a GROUP BY bulan) b on b.bulan = a.bulan_text ORDER BY a.bulan_text asc) a");
    if ($query->result()) {  
        foreach ($query->result() as $data) {
        ...
        function cari_sales_ytd_mtm_tahunini()
        {
            $query = $this->db->query("SELECT GROUP_CONCAT(eqv_idr) eqv_idr from (SELECT bulan_text,ROUND(COALESCE(eqv_idr,0),2) eqv_idr FROM (select DISTINCT bulan_text from dim_date WHERE tahun = '2025') a left join 
                (SELECT bulan,sum(eqv_idr) eqv_idr FROM(select * from tbl_data_sales where tahun = '2025') a GROUP BY bulan) b on b.bulan = a.bulan_text ORDER BY a.bulan_text asc) a");
            if ($query->result()) {  
